...
def convert_temperature(input_value, from_unit, to_unit):  # Добавляем аргументы
    try:
        ...
        return result  # Возвращаем результат для использования в тестах
    except ValueError:
...
button_decimal = tk.Button(root, text=".", command=add_decimal)
...
button_decimal.grid(row=9, column=2, padx=5, pady=5)
